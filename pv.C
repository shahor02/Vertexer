...
void pv(std::string dataDir="./data", const std::string& itstpcTrFName="o2match_itstpc.root", const std::string& tofTrFName="o2match_tof.root")
{
  if (dataDir.empty()) {
    dataDir = "./";
  }
  ...
  mITSTPCTracks = gsl::span<o2d::TrackTPCITS>(mITSTPCtracksPtr->data(),mITSTPCtracksPtr->size());
  ...
  tofChain.GetEntry(0);
  mTOFTracks = gsl::span<o2d::MatchInfoTOF>(mTOFtracksPtr->data(),mTOFtracksPtr->size());
  //
  mMeanVertex.setSigmaX2(0.1*01);
  mMeanVertex.setSigmaY2(0.1*01);
  mMeanVertex.setSigmaZ2(6.*6.);

  o2::base::GeometryManager::loadGeometry( o2::utils::concat_string(dataDir, o2::base::NameConf::getGeomFileName()) );
  o2::base::Propagator::initFieldFromGRP( o2::utils::concat_string(dataDir, o2::base::NameConf::getGRPFileName()) );
...
void createTracksPool()
{
  ...
  std::sort(mSortID.begin(),mSortID.end(),
            [](int i,int j)
            {
              return mTracksPool[i].timeEst.getTimeStamp() < mTracksPool[j].timeEst.getTimeStamp();
            });
